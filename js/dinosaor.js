...
function updateObstacles(){
    ...
    if(obstacleSpawnCounter <=0){
        if (Math.random() < 0.02) {
            ...
            let lastObstacles = canvas.width;
            for(let i = 0; i<numObstacles; i++){
                let minSpacing = 5;
                let maxSpacing = 20
                let spacing = minSpacing + Math.random() * (maxSpacing - minSpacing)
                obstacles.push({
                    x:lastObstacles + spacing,
                    width: 20 + Math.random()*10,
                    width: 20 + Math.random()*70,
                })
                lastObstacles = obstacles[obstacles.length - 1].x + obstacles[obstacle.lenght - 1].width
            }
...
function collisionDetection(){
    obstacles.forEach(function(obstacle){
        if(dinoY + 30 >= canvas.height - obstacle.height
          && dinoY <= canvas.height && 50+30 >= obstacle.x 
